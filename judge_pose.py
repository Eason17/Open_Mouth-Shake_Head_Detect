...
class OperateDetect:
    ...
    def judge_accumulation(self, action_type, act_type):
        # 全是1或0则记作无效数据
        try:
            if action_type[0] == action_type[1] == action_type[2]:
                ...
                # 检测为张嘴
                if (action_type[0] == 1) & (action_type[1] == action_type[2] == 0):
                    act_type["Open Mouth"] += 1

                # 检测点头
                if (action_type[1] == 1) | (action_type[1] == action_type[2] == 1):
                    act_type["Nod"] += 1

                # 检测摇头
                if (action_type[2] == 1) & (action_type[0] == action_type[1] == 0):
                    act_type["Shake Head"] += 1
        except TypeError:
            pass
    ...
    def detect(self, frame, marks):
        ...
        if len(rects) != 0:
            ...
            action_type = self.action_judgment((mar, nod_value, shake_value))
